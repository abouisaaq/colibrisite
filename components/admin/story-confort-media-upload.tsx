...
import {
  removeStoryConfortPhoto,
  uploadStoryConfortPhoto,
} from "@/actions/admin";
import {
  DEFAULT_STORY_CONFORT_PHOTOS,
  type StoryConfortPhotoSlot,
} from "@/lib/about-story-confort";
import { toast } from "sonner";

const LEFT_SLOTS: { slot: StoryConfortPhotoSlot; label: string }[] = [
  { slot: "left1", label: "Photo 1" },
  { slot: "left2", label: "Photo 2" },
  { slot: "left3", label: "Photo 3" },
];

const RIGHT_SLOTS: { slot: StoryConfortPhotoSlot; label: string }[] = [
...
function PhotoSlot({
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      try {
        const url = await uploadStoryConfortPhoto(slot, formData);
        onPreviewChange(url);
  ...
  function handleRemove() {
    startTransition(async () => {
      try {
        await removeStoryConfortPhoto(slot);
        onPreviewChange("");
  ...
  const displayUrl = preview || DEFAULT_STORY_CONFORT_PHOTOS[slot];
...
interface StoryConfortMediaUploadProps {
  photoUrls: Record<StoryConfortPhotoSlot, string>;
}

export function StoryConfortMediaUpload({
  photoUrls,
}: StoryConfortMediaUploadProps) {
  const [photos, setPhotos] = useState(photoUrls);

  function update(slot: StoryConfortPhotoSlot, url: string) {
    ...
    <div className="rounded-2xl border border-[#E5E7EB] bg-[#F8FAFC] p-6">
      <div className="mb-5">
        <h4 className="font-semibold text-[#111827]">
          Histoire — Confort et réconfort
        </h4>
        <p className="mt-1 text-sm text-[#6B7280]">
          Même style que Séisme : 3 photos empilées à gauche, texte au centre, 3
          photos à droite.
        </p>
      </div>
